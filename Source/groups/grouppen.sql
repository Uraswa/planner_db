...
DROP FUNCTION IF EXISTS get_group_member;
CREATE OR REPLACE FUNCTION get_group_member(
usr_id integer,
...
CREATE OR REPLACE FUNCTION add_user_to_group(usr_id integer, gr_id integer, iscreator boolean, check_group_existance boolean)
...
BEGIN
    ...
    SELECT * FROM get_group_member(usr_id, gr_id) INTO already_exist_group_member;
...
DROP FUNCTION IF EXISTS add_user_to_group_by_invitation_link;
CREATE OR REPLACE FUNCTION add_user_to_group_by_invitation_link(usr_id integer, inv_link varchar)
    RETURNS result_type AS
$$
DECLARE
    result result_type;
...
BEGIN
    ...
    IF gr_id IS NOT NULL THEN
        result := add_user_to_group(usr_id, gr_id, false, false);
        RETURN result;
    end if;
...
DROP FUNCTION IF EXISTS create_group_and_add_user;
CREATE OR REPLACE FUNCTION create_group_and_add_user(usr_id integer, name varchar, grouptype group_type)
    RETURNS result_type AS
$$
DECLARE
    invite_link           varchar;
...
BEGIN
    ...
    SELECT * INTO add_user2group_result FROM add_user_to_group(usr_id, inserted_group_id, true, false);
...
--SELECT * FROM create_group_and_add_user(1, 'group_name2', 'default');